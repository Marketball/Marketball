...
export default function MarketCard({ market, onBet, onViewDetail, isNew, isTrending, session, profile, showToast }) {
  ...
  useEffect(() => {
    ...
    const st = ScrollTrigger.create({
      trigger: cardRef.current, start: "top 92%", once: true,
      onEnter: () => gsap.to(obj, {
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
        ...
        {!isMulti && <div style={{ display: "flex", flexDirection: "column", gap: 4, flexShrink: 0 }}>
          <button className="btn-animated" onClick={e => { e.stopPropagation(); onBet(market, "yes"); }}
            ...
            <div className="prob-pct" style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 28, lineHeight: 1, color: "#10b981", letterSpacing: 1 }}><span ref={yesNumRef}>{yesPct}</span><span style={{ fontSize: 13 }}>%</span></div>
