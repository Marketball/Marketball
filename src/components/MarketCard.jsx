...
export default function MarketCard({ market, onBet, isNew, isTrending, session, profile, showToast }) {
  ...
  return <><div className="card-hover" onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)} onClick={()=>{if(showShare)setShowShare(false);else setShowStats(true);}}
    ...
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
      <div style={{ flex:1, paddingRight:12 }}>
        <div style={{ display:"flex", gap:6, marginBottom:8, flexWrap:"wrap" }}>
          ...
          {isMulti&&<span style={{ fontSize:10, fontWeight:700, color:"#f59e0b", background:"rgba(245,158,11,0.1)", padding:"2px 8px", borderRadius:20, border:"1px solid rgba(245,158,11,0.2)" }}>🎯 {options.length} {t("markets.choices")}</span>}
          {isNew&&<span style={{ fontSize:10, fontWeight:700, color:"#f97316", background:"rgba(249,115,22,0.12)", padding:"2px 8px", borderRadius:20, border:"1px solid rgba(249,115,22,0.25)", animation:"pulse 2s infinite" }}>{t("markets.new_badge")}</span>}
          {isTrending&&!isNew&&<span style={{ fontSize:10, fontWeight:700, color:"#a78bfa", background:"rgba(167,139,250,0.1)", padding:"2px 8px", borderRadius:20, border:"1px solid rgba(167,139,250,0.2)" }}>{t("markets.trending_badge")}</span>}
          <span style={{ fontSize:10, color:"rgba(241,245,249,0.3)" }}>⏱ {timeLeft(market.closes_at)}</span>
        </div>
        <div style={{ fontWeight:800, fontSize:15, color:"#f1f5f9", lineHeight:1.4, marginBottom:3 }}>{mTitle(market,lang)}</div>
      ...
      {!isMulti&&<div style={{ display:"flex", flexDirection:"column", gap:4, flexShrink:0 }}>
        <button className="btn-animated" onClick={e=>{e.stopPropagation();onBet(market,"yes");}} style={{ background:"rgba(16,185,129,0.08)", border:"1px solid rgba(16,185,129,0.2)", borderRadius:9, padding:"5px 10px", cursor:"pointer", textAlign:"center", minWidth:58 }}>
          ...
          <div style={{ fontSize:9, color:"rgba(16,185,129,0.6)", letterSpacing:1, fontWeight:700 }}>{t("common.yes")}</div>
        </button>
        <button className="btn-animated" onClick={e=>{e.stopPropagation();onBet(market,"no");}} style={{ background:"rgba(239,68,68,0.06)", border:"1px solid rgba(239,68,68,0.15)", borderRadius:9, padding:"5px 10px", cursor:"pointer", textAlign:"center", minWidth:58 }}>
          ...
          <div style={{ fontSize:9, color:"rgba(239,68,68,0.5)", letterSpacing:1, fontWeight:700 }}>{t("common.no")}</div>
        </button>
      </div>}
    </div>
    {isMulti?(
    ...
    <div style={{ display:"flex", gap:16, margin:"12px 0 14px" }}>
      ...
      <div><div style={{ fontSize:9, color:"rgba(241,245,249,0.25)", marginBottom:2, letterSpacing:1 }}>{t("markets.players")}</div><div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:15, letterSpacing:1 }}>{fmt(market.participants)}</div></div>
    </div>
